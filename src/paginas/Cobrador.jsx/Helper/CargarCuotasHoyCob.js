...
export const CargarCuotasHoyCob = async (usuarioId,setCuotasHoy, navigate) => {
    try {
      const resp = await authApi.get(`/geren/cuotasHoy-cobrador/${usuarioId}`); // Petición al backend
  
      // Validar si la respuesta contiene un array de usuarios
      if (Array.isArray(resp.data.cuotas)) {
        setCuotasHoy(resp.data.cuotas); // Actualiza el estado con los usuarios
      } else {
