...
export const starEditarProductos = async (updatedPlan,setRefreshData, navigate) => {
    try {
        const resp = await authApi.put('/geren/edit-product', updatedPlan);
       ...
       if (errores) {
           ...
           const mensajeDeError = Object.values(errores)[0]?.msg || 'Error al actualizar el producto';
           ...
           const errorMessage = error.response?.data?.msg || 'Error al actualizar el producto';
           Swal.fire({
               title: "ERROR",
               text: errorMessage,
               icon: "error",
