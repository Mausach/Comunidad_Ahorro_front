...
export const CargarCobradores = async (setCobradores, navigate) => {
    try {
      const resp = await authApi.get('/geren/cobradores'); // Petición al backend
  
      // Validar si la respuesta contiene un array de usuarios
      if (Array.isArray(resp.data.usuarios)) {
        setCobradores(resp.data.usuarios); // Actualiza el estado con los usuarios
      } else {
        console.error('Los datos del prestamo no son un array:', resp.data);
        setCobradores(resp.data.usuarios)
      }
    } catch (error) {
     // Asegúrate de acceder correctamente a los errores
                   ...
                   if (errores) {
                       ...
                       const mensajeDeError = Object.values(errores)[0]?.msg || 'Error al cargar usuarios cobradores';
                       ...
                       const errorMessage = error.response?.data?.msg || 'Error a cargar usuarios cobradores ';
                       Swal.fire({
                           title: "ERROR",
                           text: errorMessage,
                           icon: "error",
