...
export const starCrearPrestamoCliente = async (newUser,setRefreshData, navigate) => {
    try {
        const resp = await authApi.post('/geren/new-pre-client', newUser);

        Swal.fire({
            title: "Prestamo creado",
            text: "El Prestamo ha sido creado exitosamente.",
            icon: "success",
            background: "#f9f9f9",
        ...
        if (errores) {
            ...
            const mensajeDeError = Object.values(errores)[0]?.msg || 'Error al crear el Prestamo';
            ...
            const errorMessage = error.response?.data?.msg || 'Error al crear el prestamo';
            Swal.fire({
                title: "ERROR",
                text: errorMessage,
                icon: "error",
