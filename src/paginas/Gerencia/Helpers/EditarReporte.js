...
export const starEditarReporte = async (updatedReport,setRefreshData, navigate) => {
    try {
        const resp = await authApi.put('/geren/edit-reporte', updatedReport);
       ...
       if (errores) {
           ...
           const mensajeDeError = Object.values(errores)[0]?.msg || 'Error al actualizar el reporte';
           ...
           const errorMessage = error.response?.data?.msg || 'Error al actualizar el reporte';
           Swal.fire({
               title: "ERROR",
               text: errorMessage,
               icon: "error",
