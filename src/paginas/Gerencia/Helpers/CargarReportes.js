...
export const CargarReportes = async (setReportes, navigate) => {
    try {
      const resp = await authApi.get('/geren/Reportes'); // Petición al backend
  
      // Validar si la respuesta contiene un array de usuarios
      if (Array.isArray(resp.data.reportes)) {
        setReportes(resp.data.reportes); // Actualiza el estado con los usuarios
      } else {
