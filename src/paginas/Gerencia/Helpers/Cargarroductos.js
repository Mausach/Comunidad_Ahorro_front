...
export const CargarProductos = async (setProductos, navigate) => {
    try {
      const resp = await authApi.get('/geren/Productos'); // Petición al backend
  
      // Validar si la respuesta contiene un array de usuarios
      if (Array.isArray(resp.data.productos)) {
        setProductos(resp.data.productos); // Actualiza el estado con los usuarios
      } else {
