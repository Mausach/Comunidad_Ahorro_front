...
export const starEditarEquipoVenta = async (equipoEditando,setRefreshData, navigate) => {
    try {
        const resp = await authApi.put('/geren/edit-equip-venta', equipoEditando);
       ...
       if (errores) {
           ...
           const mensajeDeError = Object.values(errores)[0]?.msg || 'Error al actualizar el equipo';
           ...
           const errorMessage = error.response?.data?.msg || 'Error al actualizar el equipo';
           Swal.fire({
               title: "ERROR",
               text: errorMessage,
               icon: "error",
