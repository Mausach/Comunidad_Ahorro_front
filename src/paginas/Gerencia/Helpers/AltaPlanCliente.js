...
export const starCrearPlanCliente = async (newUser,setRefreshData, navigate) => {
    try {
        const resp = await authApi.post('/geren/new-plan-client', newUser);
        ...
        if (errores) {
            ...
            const mensajeDeError = Object.values(errores)[0]?.msg || 'Error al crear el Plan';
            ...
            const errorMessage = error.response?.data?.msg || 'Error al crear el plan';
            Swal.fire({
                title: "ERROR",
                text: errorMessage,
                icon: "error",
