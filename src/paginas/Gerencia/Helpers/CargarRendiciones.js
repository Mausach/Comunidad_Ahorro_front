...
export const CargarRendiciones = async (setRendicion, navigate) => {
    try {
        const resp = await authApi.get('/geren/rendiciones'); // Petición al backend

        // Validar si la respuesta contiene un array de usuarios
        if (Array.isArray(resp.data)) {
            setRendicion(resp.data); // Actualiza el estado con los usuarios
        } else {
        ...
        if (errores) {
            ...
            const mensajeDeError = Object.values(errores)[0]?.msg || 'Error al cargar rendiciones';
            ...
            const errorMessage = error.response?.data?.msg || 'Error al cargar rendiciones';
            Swal.fire({
                title: "ERROR",
                text: errorMessage,
                icon: "error",
