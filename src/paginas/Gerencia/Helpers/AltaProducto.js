...
export const starCrearProducto = async (nuevoProducto,setRefreshData, navigate) => {
    try {
        const resp = await authApi.post('/geren/new-product', nuevoProducto);

        Swal.fire({
            title: "Producto creado",
            text: "El Producto o plan ha sido creado exitosamente.",
            icon: "success",
            background: "#f9f9f9",
       ...
       if (errores) {
           ...
           const mensajeDeError = Object.values(errores)[0]?.msg || 'Error al crear producto';
           ...
           const errorMessage = error.response?.data?.msg || 'Error al crear producto';
           Swal.fire({
               title: "ERROR",
               text: errorMessage,
               icon: "error",
