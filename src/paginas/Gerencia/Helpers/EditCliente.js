...
export const starEditarCliente = async (updatedCliente,setRefreshData, navigate) => {
    try {

        const resp = await authApi.put('/geren//edit-cliente', updatedCliente);
       ...
       // Asegúrate de acceder correctamente a los errores
              ...
              if (errores) {
                  ...
                  const mensajeDeError = Object.values(errores)[0]?.msg || 'Error al actualizar el cliente';
                  ...
                  const errorMessage = error.response?.data?.msg || 'Error al actualizar el cliente';
                  Swal.fire({
                      title: "ERROR",
                      text: errorMessage,
                      icon: "error",
