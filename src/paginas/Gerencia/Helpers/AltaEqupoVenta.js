...
export const starCrearEquipoVenta = async (newEquipo,setRefreshData, navigate) => {
    try {
        const resp = await authApi.post('/geren/new-equipo-venta', newEquipo);
        ...
        if (errores) {
            ...
            const mensajeDeError = Object.values(errores)[0]?.msg || 'Error al crear el Equipo';
            ...
            const errorMessage = error.response?.data?.msg || 'Error al crear el Equipo';
            Swal.fire({
                title: "ERROR",
                text: errorMessage,
                icon: "error",
